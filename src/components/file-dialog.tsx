	...
	return (
		<Dialog>
			...
			<DialogContent className="sm:max-w-[480px]">
				<p className="absolute text-base font-medium left-5 top-4 text-muted-foreground">Sube la foto de tu factura</p>
				<div
					{...getRootProps()}
					className={cn(
...
function FileCard<TFieldValues extends FieldValues>({ i, file, name, setValue, files, setFiles }: FileCardProps<TFieldValues>) {
	...
	return (
		<div className="relative flex items-center justify-between gap-2.5">
			<div className="flex items-center gap-2">
				<Image src={cropData ? cropData : file.preview} alt={file.name} className="w-10 h-10 rounded-md shrink-0" width={40} height={40} loading="lazy" />
			...
			<div className="flex items-center gap-2">
				{file.type.startsWith('image') && (
					<Dialog open={isOpen} onOpenChange={setIsOpen}>
						<DialogTrigger asChild>
							<Button type="button" variant="outline" size="sm" className="p-0 h-7 w-7">
								<Icons.crop className="w-4 h-4 text-white" aria-hidden="true" />
								<span className="sr-only">Crop image</span>
							</Button>
						</DialogTrigger>
						<DialogContent>
							<p className="absolute text-base font-medium left-5 top-4 text-muted-foreground">Crop image</p>
				...
				<Button
					...
					onClick={() => {
						...
						setValue(name, files.filter((_, j) => j !== i) as PathValue<TFieldValues, Path<TFieldValues>>, {
							shouldValidate: true,
						});
					}}
				>
					<Icons.close className="w-4 h-4 text-white" aria-hidden="true" />
